...
#include "koji_dummy/koji_publisher.hpp" // ヘッダのパスをパッケージ名／クラス名に合わせる
...
constexpr char VIDEO_FILENAME[] = "/media/koji.mp4";
...
KojiPublisher::KojiPublisher(const rclcpp::NodeOptions &node_options)
    : Node("koji_publisher", node_options), stop_thread_(false) {
    // 動画ファイルパスを取得
    try {
        video_path_ = ament_index_cpp::get_package_share_directory("koji_dummy") + VIDEO_FILENAME;
    } catch (...) {
    ...
    // パブリッシュスレッドを開始
    publish_thread_ = std::thread(&KojiPublisher::publishLoop, this);
}

KojiPublisher::~KojiPublisher() {
    // スレッドを停止
    stop_thread_ = true;
...
void KojiPublisher::createPublishers() {
    // QoS設定
...
void KojiPublisher::publishLoop() {
    // 動画ファイルを開く
...
#include "rclcpp_components/register_node_macro.hpp"
RCLCPP_COMPONENTS_REGISTER_NODE(koji_dummy::KojiPublisher)
